...
SPC_USE_ARCH=aarch64 ./bin/spc-alpine-docker download --with-php="8.4" --for-extensions="ctype,curl,dom,filter,libxml,mbstring,openssl,pcntl,phar,posix,simplexml,sockets,xml,xmlwriter,zlib" --prefer-pre-built
SPC_USE_ARCH=aarch64 ./bin/spc-alpine-docker switch-php-version "8.4"
SPC_USE_ARCH=aarch64 ./bin/spc-alpine-docker build --build-micro "ctype,curl,dom,filter,libxml,mbstring,openssl,pcntl,phar,posix,simplexml,sockets,xml,xmlwriter,zlib"
